#ifndef EOS_CD_MODIFIED_H
#define EOS_CD_MODIFIED_H
...
class EoS_cd_modified: public EoS_base
{
private:

  double c, T0, muB0, muS0, muQ0, cB, cQ, cS;
...
public:
  ...
  EoS_cd_modified(){std::cout << "In default constructor!\n"; abort();}
  virtual ~EoS_cd_modified(){}

  EoS_cd_modified( const double c_in,
                          const double T0_in, const double muB0_in,
                          const double muS0_in, const double muQ0_in,
                          const double cB_in, const double cQ_in, const double cS_in,
  ...
  double p(double T, double muB, double muQ, double muS)
    ...
    double sum1 = cB*(T/T0)*(T/T0)*(muB/muB0)*(muB/muB0);
    double sum2 = cS*(T/T0)*(T/T0)*(muS/muS0)*(muS/muS0);
    double sum3 = cQ*(T/T0)*(T/T0)*(muQ/muQ0)*(muQ/muQ0);
    double cp = c*T0*T0*T0*T0;  // c dimensionless, cp - 1/fm^4
		return cp*(x + sum1 + sum2 + sum3);
	}
